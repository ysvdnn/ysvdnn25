...
Potion Potion::operator*(const Potion& other) const {
    ...
    for (int i = 0; i < ingredients.size(); i++) {
        for (int j = 0; j < other.ingredients.size(); j++) {
            if (ingredients[i] == other.ingredients[j]) {
                ...
                for (int k = 0; k < newIngredients.size(); k++) {
                    if (newIngredients[k] == ingredients[i]) {
                        alreadyAdded = true;
                        break;
                    }
                }
                if (!alreadyAdded) newIngredients.push_back(ingredients[i]);
                break;
            }
        }
    }
    ...
    return Potion("Микс", newBase, newVolume, newIngredients);
}
